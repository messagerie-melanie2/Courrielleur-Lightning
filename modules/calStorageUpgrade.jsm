...
function upgradeDB(db) {
    ...
    if (db.tableExists("cal_calendar_schema_version")) {
        ...
        if (version < DB_SCHEMA_VERSION) {
            ...
            // CM2V6 -MANTIS 3761: Le backup des sqlite de calendar-data empeche l'ajout de l'agenda
            //backupDB(db, version);
...
// CM2V6
/**
 * Bugzilla 168680 - Add binary attachments support
 * Fix saving value for looking binary/url attachments
 */
upgrade.v20 = function upgrade_v20(db, version) {
    ...
    beginTransaction(db);
    try {
        addColumn(tbl, "cal_attachments", "value", "TEXT", db);
...
/**
 * Bugzilla 168680 - Add hashid and has changed stockage in db
 * Used for cached attachments
 */
upgrade.v21 = function upgrade_v21(db, version) {
    ...
    beginTransaction(db);
    try {
        addColumn(tbl, "cal_attachments", "hash_id", "TEXT", db);
        addColumn(tbl, "cal_attachments", "size", "INTEGER", db);
 ...
 * r=mmecca, p=philipp
 */
upgrade.v24 = function(db, version) {
    let tbl = upgrade.v23(version < 23 && db, version);
    LOGdb(db, "Storage: Upgrading to v24");
    beginTransaction(db);
    try {
...
//CM2V7 MANTIS 0004494: Mettre à  jour le href dans les metadata
/**
 * MANTIS 0004494 - Mettre à  jour la table meta data avec le nouveau href
 */
upgrade.v25 = function upgrade_v25(db, version) {
    let tbl = upgrade.v24(version < 24 && db, version);
    LOGdb(db, "Storage: Upgrading to v25");
    beginTransaction(db);
    try {
    	// Mise à  jour des href pour les meta data
    	executeSimpleSQL(db, "UPDATE cal_metadata SET value = REPLACE(value, '/calendrier/caldav.php/', '/davm2/caldav.php/calendars/')");
      setDbVersionAndCommit(db, 25);
    } catch (e) {
        throw reportErrorAndRollback(db,e);
    }
    return tbl;
