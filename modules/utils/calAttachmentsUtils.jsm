...
var calattachments = {
    ...
    formatBinaryAttachment: function(aAttachment, aCallback) {
	    var filename = this.makeAttachPrettyName(aAttachment);
    ...
    // version de makePrettyName qui prend en paramètre un attachment d'événement
    // utilise le nom de la propriété "X-MOZILLA-CALDAV-ATTACHMENT-NAME" si existe
    // sinon idem makePrettyName
    makeAttachPrettyName: function(attachment){

      let attachName=attachment.getParameter("X-MOZILLA-CALDAV-ATTACHMENT-NAME");

      if (null==attachName)
        attachName=this.makePrettyName(attachment.uri);
