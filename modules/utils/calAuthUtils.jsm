...
var calauth = {
    ...
    Prompt: class {
        ...
        getPasswordInfo(aPasswordRealm) {
    		...
    		// CMel - Autentification multiple pour les agendas
            let logins = Services.logins.findLogins({}, aPasswordRealm.prePath, aPasswordRealm.formSubmitURL, aPasswordRealm.realm);
            // Fin CMel
            if (logins.length) {
            ...
            if (found) {
                // CMel -MANTIS 3195: Demandes d'authentifications liées à la désactivation d'alarmes avant l'authentification Pacome
                //let keyStr = aPasswordRealm.prePath +":" + aPasswordRealm.realm;
        		let keyStr = aPasswordRealm.formSubmitURL;
        		// Fin CMel
                let now = new Date();
        ...
        promptAuth(aChannel, aLevel, aAuthInfo) {
            ...
            // CMel - Authentification multiple des calendriers
            hostRealm.formSubmitURL = aChannel.URI.spec;
            // Fin CMel
