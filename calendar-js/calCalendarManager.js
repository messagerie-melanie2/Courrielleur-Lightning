...
calCalendarManager.prototype = {
    ...
    setupCalendar: function(calendar) {
        ...
        cmelSynchroAgenda.setupCalendarTimer(calendar);
    },
    ...
    unregisterCalendar: function(calendar) {
        ...
        cmelSynchroAgenda.clearCalendarTimer(calendar);
    },
    ...
    getCalendars: function(count) {
        ...
        let calendars = [];
        // CMel
        let sortOrderPref = Preferences.get("calendar.list.sortOrder", null);
        if (sortOrderPref) {
            sortOrderPref = sortOrderPref.split(" ");
            for (let id of sortOrderPref) {
                let calendar = this.mCache[id];
    ...
    onPropertyChanged: function(aCalendar, aName, aValue, aOldValue) {
        ...
        switch (aName) {
            ...
            case "refreshInterval":
                ...
                cmelSynchroAgenda.setupCalendarTimer(aCalendar);
                break;
            case "cache.enabled":
            ...
            case "disabled":
                if (!aValue && aCalendar.canRefresh) {
                	cmelSynchroAgenda.refreshCalendar(aCalendar);
                    //aCalendar.refresh();
                }
                break;
        }
    ...
    changeCalendarCache: function(aCalendar, aName, aValue, aOldValue) {
        ...
        if (aOldValue != aValue) {
            ...
            let propsToCopy = [
                ...
                "readOnly",
		// CMel - MANTIS 3766: Perte de la propriété pacome de l'agenda à la décoche du cache
                "pacome",
                // Fin CMel
                "imip.identity.key"
            ];
