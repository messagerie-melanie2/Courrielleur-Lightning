...
webDavSyncHandler.prototype = {
    ...
    onStartRequest: function(request, context) {
        ...
        else 
        {
            ...
            if(responseStatus == 401 && deltaMinute >= 1)
            {
...
multigetSyncHandler.prototype = {
    ...
    doMultiGet: function() {
        ...
        this.calendar.sendHttpRequest(requestUri, queryXml, MIME_TEXT_XML, null, (channel) => {
            channel.requestMethod = "REPORT";
            channel.setRequestHeader("Depth", "1", false);
            // CM2V6 Attachments - 18/08/2011 - Gestion des pièces jointes côté serveur
        	if (Preferences.get("calendar.attachments.active", true) 
          			&& Preferences.get("calendar.attachments.caldav.compatible", true)) {
        		channel.setRequestHeader("X-MOZ-Attachments", "1", false);
        	}
