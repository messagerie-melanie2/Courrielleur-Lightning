...
calRecurrenceInfo.prototype = {
    ...
    calculateDates: function(aRangeStart, aRangeEnd, aMaxCount) {
        ...
        // CMel MANTIS 2588: La recherche ne retourne pas tous les résultats liés à une récurrence
        if (!aRangeStart) {
        	aRangeStart = this.mBaseItem.recurrenceStartDate;
        }
        // Fin CMel MANTIS 2588: La recherche ne retourne pas tous les résultats liés à une récurrence
        ...
        for (let ritem of this.mPositiveRules) {                                   
    		...
    		if (!rangeEnd) {
    			cur_dates = ritem.getOccurrences(startDate,
                        ...
                        0,
                        {});
    		} else { // Fin CMel MANTIS 2588: La recherche ne retourne pas tous les résultats liés à une récurrence
    			cur_dates = ritem.getOccurrences(startDate,
                         searchStart,
                         rangeEnd,
                         maxCount, {});
        ...
        for (let ritem of ritems) {
            ...
            if (rDateInstance) {
                ...
                if (!ritem.isByCount) {
                    ...
                    if (untilDate) {
                    	// CMel MANTIS 3254: Erreur lors de la modification de la date de début d'un évènement hebdomadaire
                    	let untilDateUTC = untilDate.clone().getInTimezone(cal.dtz.UTC);
                    	let aNewStartTimeUTC = aNewStartTime.clone().getInTimezone(cal.dtz.UTC);
                    	// CMel MANTIS 2367: Disparition d'une fin d'occurrence
                    	if (untilDateUTC.hour != aNewStartTimeUTC.hour
	                    		|| untilDateUTC.minute != aNewStartTimeUTC.minute
	                    		|| untilDateUTC.seconde != aNewStartTimeUTC.seconde) {
