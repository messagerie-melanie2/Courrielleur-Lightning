...
calMonthPrinter.prototype = {
    ...
    formatToHtml: function(aStream, aStart, aEnd, aCount, aItems, aTitle) {
        ...
        // liste des mois pour setupListeAgenda (0 à 11)
        let mois=[];
        ...
        // et des categories
        for (var m of mois){
          this.setupListeAgenda(document, agendas, couleurAgendas, m);

          this.setupListeCategories(document, categories, m);
        }
    ...
    setupMonth: function(document, startOfMonth, dayTable) {
        ...
        let ident="mois-"+startOfMonth.month;
        currentMonth.setAttribute("id", ident);
        ...
        for (let i = 1; i <= 7; i++) {
            ...
            dayTitle.textContent = cal.l10n.getDateFmtString(`day.${dayNumber}.name`);
        }
    ...
    setupListeAgenda: function(document, agendas, couleurAgendas, mois){

      if (0==agendas.length)
        return;

      let moisContainer=document.getElementById("mois-"+mois);
      let agendasContainer=moisContainer.querySelector(".agendas-container");
    ...
    setupListeCategories: function(document, categories, mois){

      if (0==categories.length)
        return;

      let moisContainer=document.getElementById("mois-"+mois);
      let categoriesContainer=moisContainer.querySelector(".categories-container");
