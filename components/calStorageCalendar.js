...
calStorageCalendar.prototype = {
    ...
    deleteCalendar: function(aCalendar, listener) {
        ...
        // CM2V6 - Un sqlite par calendar
        let calDBFile=cal.provider.getCalendarDirectory();
        calDBFile.append(this.id+".sqlite");
        if (calDBFile.exists() &&
            calDBFile.equals(this.mDB.databaseFile)){
          try {
            cal.LOG("[calStorageCalendar] deleting calendar file:"+calDBFile.path);
            calDBFile.remove(true);
          } catch(ex) {
            this.logError("Error deleting calendar file", ex);
          }
        }
        // Fin CM2V6 - Un sqlite par calendar
    ...
    prepareInitDB: function() {
        if (this.uri.schemeIs("file")) {
            // CM2V6 - Gestion DB
            let fromDBFile = cal.provider.getCalendarDirectory();
            fromDBFile.append("cache.sqlite");
            this.migrateCalendarDB(fromDBFile);
            // Fin CM2V6 - Gestion DB
    ...
    // CM2V6 - Une base de donnees par calendrier
    /**
     * Create the single database for the calendar if needed
     *  and copy the original data from fromDBFile file database if exist
     *  if migration fail, the original database is used
     * If the database for the calendar exist, open it
     */
    migrateCalendarDB: function(fromDBFile) {

      let calDBFile=cal.provider.getCalendarDirectory();
      calDBFile.append(this.id+".sqlite");

      let bNewDB = !calDBFile.exists();
      if (bNewDB) {
        // open the original database if exists
        if (fromDBFile.exists()) {
          this.mDB = Services.storage.openDatabase(fromDBFile);
          upgradeDB(this.mDB);
        } else {
          bNewDB=false;
        }
      ...
      //copy calendar data from "cache.sqlite" to <this.id>".sqlite"
      if (bNewDB) {
        let calDB;

        try {
          //migrate to 1 database per calendar
          cal.LOG("[calStorageCalendar] Migrating to 1 database per calendar.");

          // open the new database
          calDB = Services.storage.openDatabase(calDBFile);
          // CM2V6 - Execution a la creation de la base
          calDB.executeSimpleSQL("PRAGMA journal_mode=WAL");

          upgradeDB(calDB);

          try {
            this.mDB.executeSimpleSQL("ATTACH DATABASE '"+calDB.databaseFile.path+"' AS caldb");
          } catch (ex) {throw ex;}
          ...
          try {
            ...
            try {
              ...
              for (let table of ["cal_alarms", "cal_attachments",
                                      "cal_attendees", "cal_events",
                                      "cal_metadata", "cal_properties",
                                      "cal_recurrence", "cal_relations",
                                      "cal_todos"]) {
                                       
                this.mDB.executeSimpleSQL("INSERT INTO caldb." +  table +
                                          " SELECT * FROM "+ table +
                                          " WHERE cal_id='" + this.id +"'");
              }
          ...
          this.logError("prepareInitDB can't migrate to 1 database per calendar failed.", ex);
          calDB.close();
          cal.LOG("prepareInitDB removing new database file:"+calDBFile.path);
          calDBFile.remove(false);
        }

      } else {

        this.mDB = Services.storage.openDatabase(calDBFile);

        upgradeDB(this.mDB);
      }
    },
    // Fn CM2V6 - Une base de donnees par calendrier
    ...
    getItems_: function(aItemFilter, aCount, aRangeStart, aRangeEnd, aListener) {
        ...
        if (wantEvents) {
            ...
            try {
                ...
                // CM2V6 - MANTIS 3765: Erreur lors de la suppression d'un agenda
