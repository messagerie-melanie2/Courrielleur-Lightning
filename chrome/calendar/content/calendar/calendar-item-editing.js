...
function createEventWithDialog(calendar, startDate, endDate, summary, event, aForceAllday) {
    ...
    if (event) {
        ...
        // CM2V6 - MANTIS 2896: Le statut devrait être confirmé par défaut
...
function openEventDialog(calendarItem, calendar, mode, callback, job=null, initialDate=null, counterProposal) {
    ...
    // MANTIS 0005518: Choix de l'agenda par défaut lors de la création d'un événement
...
function promptOccurrenceModification(aItem, aNeedsFuture, aAction) {
    ...
    if (items.every(item => item == item.parentItem)) {
        ...
        // 5648 Permet de supprimer le dernier évènement d'une récurence
        if(items.length == 1 && items[0].parentItem.title == null)
        {
            if(aAction == "edit")
