    ...
    switch (flavor.value) {
        ...
        case "text/unicode": {
            ...
            if (!pasteIntoSelectedCalendar) {
                ...
                pasteIntoSelectedCalendar = cal.acl.isCalendarWritable(selectedCal) 
                    && cal.acl.userCanAddItemsToCalendar(selectedCal)
                    && ((currView == "agenda" 
                            || currView == "week-agenda" 
                            || currView == "week2-agenda")
                        || (!selectedCal.getProperty("disabled") 
                            && selectedCal.getProperty("calendar-main-in-composite")));
            }
            if (pasteIntoSelectedCalendar) {
            ...
            for (let item of items) {
                ...
                // CMel
                if (destCal.getProperty("pacome")) newItem.setProperty("X-CM2V3-ACTION","COPY");
                // Fin CMel
