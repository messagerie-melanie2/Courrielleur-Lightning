...
/**
 * Calls the print dialog to edit the currently selected item
 */
agendaListbox.printSelectedItem = function() {
    let listItem = document.getElementById("agenda-listbox").selectedItem;
    if (listItem) {
        calItemPrint(listItem.occurrence);
    }
};
...
agendaListbox.setupContextMenu = function(popup) {
    ...
    agendaListbox.cm2InitReponse(agendaListbox.getSelectedItems({}));
    agendaListbox.cm2InitOptionPrive(agendaListbox.getSelectedItems({}));
};

/**
 * CMel - MANTIS 0005511: Reprendre le menu contextuel d'événements dans le panneau "Aujourd'hui"
 * initialise le menu reponse
 * @param items         An array of items (usually the selected items) to adapt
 *                        the context menu for.
 */
agendaListbox.cm2InitReponse = function(items) {
    if (items.length != 1){
      return;
    }
    ...
    if (cal.acl.isCalendarWritable(calendar) 
            && cal.itip.isInvitation(item) 
            && cal.acl.userCanModifyItem(item)) {
        document.getElementById("agenda-context-menu-attend-accept-menuitem").setAttribute("hidden", "false");
        document.getElementById("agenda-context-menu-attend-tentative-menuitem").setAttribute("hidden", "false");
        document.getElementById("agenda-context-menu-attend-declined-menuitem").setAttribute("hidden", "false");
        document.getElementById("agenda-context-menuseparator-reponse").setAttribute("hidden", "false");
        agendaListbox.cm2SetCocheReponse(item);
    }
};
 ...
 */
agendaListbox.cm2SetCocheReponse = function(item) {
    let calendar = item.calendar;
    let attendee = calendar.getInvitedAttendee(item);
    let reponse = attendee.participationStatus;
    let option = document.getElementById("agenda-context-menu-attend-accept-menuitem");
    option.setAttribute("checked", (reponse == "ACCEPTED" ? true : false));
    option=document.getElementById("agenda-context-menu-attend-tentative-menuitem");
    option.setAttribute("checked", (reponse == "TENTATIVE"? true : false));
    option=document.getElementById("agenda-context-menu-attend-declined-menuitem");
 ...
 *                        the context menu for.
 */
agendaListbox.cm2InitStatut = function(items) {
    if (items.length != 1) {
      return;
    }
    ...
    let option = document.getElementById("agenda-context-menu-statut-provisoire");
    let etat = statut == "TENTATIVE" ? true : false;
    option.setAttribute("checked", etat);
    if (etat) on = true;
  
    option = document.getElementById("agenda-context-menu-statut-occupe");
    etat = statut == "CONFIRMED" ? true : false;
    option.setAttribute("checked", etat);
    if (etat) on = true;
  
    option = document.getElementById("agenda-context-menu-statut-annule");
    etat = statut == "CANCELLED" ? true : false;
    option.setAttribute("checked", etat);
    if (etat) on = true;
  
    option = document.getElementById("agenda-context-menu-statut-libre");
 ...
 */
agendaListbox.cm2InitOptionPrive = function(items) {
    if (items.length != 1) {
      return;
    }
    ...
    let option = document.getElementById("agenda-context-menu-evprive");
    option.setAttribute("checked", (privacy == "PRIVATE" ? true : false));
};
  
agendaListbox.cm2ModifyPrive = function(items) {
    if (items.length != 1) {
      return;
    }
 ...
 */
agendaListbox.cm2ModifyStatut = function(items, statut) {
    let nb = items.length;
    for (let i=0; i<nb; i++) {
        let item=items[0];
        let newItem=item.clone();
 ...
 */
agendaListbox.cm2ModifyReponse = function(items, type) {
