...
var calendarViewController = {
    ...
    createNewEvent: function(aCalendar, aStartTime, aEndTime, aForceAllday) {
        ...
        // CM2V6 - MANTIS 2899: Proposition: Un glisser/déplacer pour créer un évènement ouvre la boite de dialogue
        ...
        //} else {
            createEventWithDialog(aCalendar, aStartTime, aEndTime, null, null, aForceAllday);
        //}
        // Fin CM2V6
    },
     ...
     */
    // CM2V6
    modifyOccurrence: function (aOccurrence, aNewStartTime, aNewEndTime, aNewTitle, aNewCalendar) {
    // Fin CM2V6
        ...
        if (aNewStartTime || aNewEndTime || aNewTitle) {
            ...
            // CM2V6 - Bugzilla 168680 - Cursor and status bar
            window.setCursor("wait");
            // Show message on status bar      
            gEventStatusFeedback.initialize(window);
            gEventStatusFeedback.showStatusString(cal.l10n.getCalString("modifyCurrentEvent"));
            
            var listener = {
                onOperationComplete: function modifyOccurrence_ooc(aCalendar, aStatus,
                                                            aOperationType, aId,
                                                            aDetail) {
                    window.setCursor("auto");
                    if (!Components.isSuccessCode(aStatus)) {
                      gEventStatusFeedback.showStatusString(cal.l10n.getCalString("errorModifyCurrentEvent"));
                    }
                    else { 
            ...
            // Gestion du drag n drop d'un agenda à un autre
            if (aNewCalendar && instance.calendar != aNewCalendar) {
            	if (instance.calendar.getProperty("pacome")) instance.setProperty("X-CM2V3-ACTION","MOVE");
            	instance.calendar = aNewCalendar;
            }
            else {
            	if (instance.calendar.getProperty("pacome")) instance.setProperty("X-CM2V3-ACTION","MODIFY");
            }
            doTransaction("modify", instance, instance.calendar, aOccurrence, listener);
            // Fin CM2V6
    ...
    deleteOccurrences: function(aCount,
        ...
        for (let itemToDelete of occurrences) {
            ...
            if (itemToDelete.parentItem.hashId == itemToDelete.hashId) {
                ...
                // Show message on status bar      
                gEventStatusFeedback.initialize(window);
                if (occurrences.length <= 1) gEventStatusFeedback.showStatusString(cal.l10n.getCalString("deleteCurrentEvent"));
                else gEventStatusFeedback.showStatusString(cal.l10n.getCalString("deleteCurrentsEvents"));
                ...
                var listener = {
                    onOperationComplete: function modifyOccurrence_ooc(aCalendar, aStatus,
                        ...
                        window.setCursor("auto");
                        if (!Components.isSuccessCode(aStatus)) {
                          if (occurrences.length <= 1) gEventStatusFeedback.showStatusString(cal.l10n.getCalString("errorDeleteCurrentEvent"));
                          else gEventStatusFeedback.showStatusString(cal.l10n.getCalString("errorDeleteCurrentsEvents"));
                        }
                        else { 
                          gEventStatusFeedback.showStatusString("");
                        }
