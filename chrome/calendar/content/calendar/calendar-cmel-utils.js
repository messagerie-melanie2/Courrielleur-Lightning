  ...
  try {
    ...
    if (documentLink.selectedItems.length == 1) {
      ...
      let myurl = Preferences.get("calendar.attachments.url.melanie2web",
                                  "https://mceweb2.si.minint.fr/services/download/");
...
function saveAsAttachment() {
  ...
  fp.defaultString = cal.attachments.makeAttachPrettyName(attachment);
...
// version avec href de openAttachment
function openAttachmentHREF(href, filename=null) {
  ...
  try {
    ...
    attachment.uri = Services.io.newURI(href);
    attachment.setParameter("X-MOZILLA-CALDAV-ATTACHMENT-NAME", filename);

    // Melanie2Web link to download attachment
    let pref_pj=Preferences.get("calendar.attachments.url.melanie2web", "");

    if (pref_pj!="" && attachment.uri.spec.indexOf(pref_pj) == 0) {
