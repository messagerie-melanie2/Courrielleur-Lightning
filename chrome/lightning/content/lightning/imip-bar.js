 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
 ...
 //#8402: Lorsque plusieurs ICS sont présents dans une invitation (PJs, cas particuliers, transferts etc), le mauvais ICS est chargé
 let lastItipLoad = new Date(0).getTime();
 ...
 var ltnImipBar = {
     ...
     observe: function(subject, topic, state) {
         //#8402: Lorsque plusieurs ICS sont présents dans une invitation (PJs, cas particuliers, transferts etc), le mauvais ICS est chargé
         // On mesure le temps écoulé depuis le dernier chargement, s'il est trop court, alors on a été appelé plusieurs fois de suite et on ne fait rien.
         let elapsedSinceLastItipLoad = new Date().getTime() - lastItipLoad;
         //console.log("elapsedSinceLastItipLoad: "+elapsedSinceLastItipLoad);
         //console.log("imip-bar observe last load: "+ elapsedSinceLastItipLoad +"ms away");

         if (topic == "onItipItemCreation" && (elapsedSinceLastItipLoad >= 500) ) {
             //console.log("imip-bar observe: Creating itip");
             lastItipLoad = new Date().getTime();
             let itipItem = null;
             let msgOverlay = null;
             try {
                 if (!subject) {
