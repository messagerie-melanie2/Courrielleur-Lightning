...
var gWarning = false;
//CM2V6 - Bugzilla 168680 - Status bar event
//var gWarning = false;
var gEventStatusBar = null;
// Fin CM2V6
...
function receiveMessage(aEvent) {
    ...
    switch (aEvent.data.command) {
        ...
        // CMel
        case "attachFile": attachFile(); break;
        case "attachFileURI": attachFileURI(); break;
        // Fin CMel
...
function loadDialog(aItem) {
    ...
    if (gNewItemUI) {
        ...
        let unfilteredList = cal.getCalendarManager().getCalendars({});
...
// CM2V6
/**
 * Prompts the user to attach a file uri to this item.
 */
function attachFileURI() {
    const nsIFilePicker = Components.interfaces.nsIFilePicker;
    let filePicker = Components.classes["@mozilla.org/filepicker;1"]
                               .createInstance(nsIFilePicker);
    filePicker.init(window,
...
function addAttachment(attachment) {
    ...
    if (attachment.uri) {
        ...
        cellItem.setAttribute("label", cal.attachments.makeAttachPrettyName(attachment));
        cellItem.setAttribute("crop", "end");
        cellItem.setAttribute("class", "listitem-iconic");
        cellItem.setAttribute("tooltiptext", attachment.uri.spec);
        ...
        if (attachment.uri.schemeIs("file")) {
          ...
          if (attachment.uri.spec.indexOf(myurl) == 0) {
            cellItem.setAttribute("image", "moz-icon://." + cal.attachments.makeAttachPrettyName(attachment).split('.').pop() + "?size=16");
          } else {
    ...
    if (repeatValue == "none") {
        ...
        switch (repeatValue) {
            ...
            case "weekly":
                ...
                break;
            case "every.weekday":
                // CMel - MANTIS 2375 - Mauvaise gestion des répétitions pour les jours ouvrables
                //recRule.type = 'DAILY';
                recRule.type = 'WEEKLY';
                // Fin CMel
...
function saveEventStatus () {
  ...
  if (saveAndCloseKey) {
	  saveAndCloseKey.disabled = true;
  }
  if (saveAndCloseKey2) {
	  saveAndCloseKey2.disabled = true;
  }
  if (saveKey) {
	  saveKey.disabled = true;
  }
  if (itemSaveAndClose) {
	  itemSaveAndClose.disabled = true;
  }
  if (itemSave) {
...
function endEventStatus (aIsError) {
  ...
  if (saveAndCloseKey) {
	  saveAndCloseKey.disabled = false;
  }
  if (saveAndCloseKey2) {
	  saveAndCloseKey2.disabled = false;
  }
  if (saveKey) {
	  saveKey.disabled = false;
  }
  if (itemSaveAndClose) {
	  itemSaveAndClose.disabled = false;
  }
  if (itemSave) {
  ...
  try {
    ...
    if (item.calendar.getProperty("pacome")){
      ...
      if (window.mode == "new") {
        ...
        if (item.calendar.id != originalItem.calendar.id){
          // deplacement
          item.setProperty("X-CM2V3-ACTION","MOVE");
        } else {
          // participation
          let oldCal = originalItem.calendar.QueryInterface(Components.interfaces.calISchedulingSupport);
          let attOld = oldCal.getInvitedAttendee(originalItem);
          let newCal = item.calendar.QueryInterface(Components.interfaces.calISchedulingSupport);
          let attNew = newCal.getInvitedAttendee(item);
          if (attOld && attNew && attOld.participationStatus!=attNew.participationStatus){
            item.setProperty("X-CM2V3-ACTION","REPLY");
          } else {
            // simple modification
            item.setProperty("X-CM2V3-ACTION","MODIFY");
          }
        }
      }
    }
