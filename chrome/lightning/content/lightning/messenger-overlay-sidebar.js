...
// CMel MANTIS 0004051: Synchroniser les agendas lorsque l'on arrive sur l'onglet Agenda
var gLastTabSwitchCalendarRefresh = 0;
...
function ltnSwitch2Calendar() {
    if (gCurrentMode != "calendar") {
        ...
        // CMel MANTIS 0004051: Synchroniser les agendas lorsque l'on arrive sur l'onglet Agenda
        if (0==gLastTabSwitchCalendarRefresh &&
            0!=cmelSynchroAgenda._starttime)
          gLastTabSwitchCalendarRefresh=cmelSynchroAgenda._starttime*1000;
        if (0==gLastTabSwitchCalendarRefresh) 
          return;
        
        let current_time = Date.now();
        if (current_time - gLastTabSwitchCalendarRefresh > 30000) {
          cal.view.getCompositeCalendar(window).refresh();
          gLastTabSwitchCalendarRefresh = current_time;
        }
    }
}
